...
用户在头条刷新时，扩展自动滚动收集前 N 条候选卡片（N 由后端 `REORDER_TOP_N` 控制，默认20），一次性发给 `/reorder`。所有加载到页面的卡片都通过 `/browse` 逐条上报浏览记录（不限于参与重排的 top_n 条）：

```
候选卡片列表（标题 + 来源账号 + 发布时间）
    ↓
阶段1（过滤）：LLM 按用户"我不想看xx"规则移除不符合内容，只返回保留/移除的 id
    ↓
阶段2（重排）：LLM 按用户"我想看xx"规则将匹配内容排前，只返回排序后的 id 列表
    ↓
返回排序后的 id 列表
```
...
| `/get_reorder_config`    | GET  | 获取重排配置（top_n 条数），前端自动滚动收集用               |
...
| `models.py` | 数据库模型（Record、Rule、Session、Message、Personalities、ReorderLog 等） |
| `urls.py`   | 路由注册                                               |
...
| 路径                            | 说明                                                 |
| ----------------------------- | -------------------------------------------------- |
| `pipeline.py`                 | 主流程：LLM 过滤 + 重排，`run_two_stage_reorder()`，结果写入 ReorderLog |
